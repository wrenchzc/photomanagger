...
class TestDisplayImg(object):
    ...
    def test_date_filter_single(self):
        ...
        command_list = CommandList(cmd_inx_test_root, {"filters": ["date.gt:2020-08-12"]})
        image_infos = command_list.get_filter_images()
        assert (len(image_infos) == 1)
        image_info = image_infos[0]
        assert image_info["filename"] == "test4.jpg"

        command_list = CommandList(cmd_inx_test_root, {"filters": ["date.gte:2020-08-12"]})
        image_infos = command_list.get_filter_images()
        assert (len(image_infos) == 2)
        image_info = image_infos[0]
        assert image_info["filename"] == "test4.jpg"
        assert image_infos[1]["filename"] == "test2.jpg"
